	...
	app := &cli.App {
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:     "source",
				Aliases:  []string{"s"},
				Usage:    "Folder źródłowy do monitorowania",
				Required: true,
			},
			&cli.StringFlag{
				Name:     "backup",
				Aliases:  []string{"b"},
				Usage:    "Folder docelowy dla kopii zapasowych",
				Required: true,
			},
			&cli.IntFlag{
				Name:    "versions",
				Aliases: []string{"v"},
				Usage:   "Maksymalna liczba wersji do przechowywania per plik",
				Value:   3,
			},
			&cli.DurationFlag{
				Name:    "interval",
				Aliases: []string{"i"},
				Usage:   "Minimalny odstęp między backupami tego samego pliku",
				Value:   5 * time.Second,
			},
		},
		Action: runWatcher,
